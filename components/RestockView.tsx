...
export default function RestockView({ data, rem, allRooms }: RestockViewProps) {
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="bg-charcoal rounded-2xl px-4 py-3 flex items-center justify-between">
        ...
        return (
          <div key={k} className="bg-white rounded-2xl p-4 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="bg-charcoal text-white rounded-full px-3 py-1 text-[15px] font-bold">
                {items.length}本
              </span>
            </div>
            <div className="flex flex-wrap gap-2">
              {items.map((it, j) => (
                ...
                >
                  <span className="text-[11px] font-bold text-white bg-[#555] rounded px-1.5 py-0.5">{it.lbl}</span>
                  <span className="text-xs font-semibold text-gray-500">{it.rn}号室</span>
                </div>
              ))}
            </div>
          </div>
