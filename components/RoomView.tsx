...
export default function RoomView({ rn, slots, isRemoved, onTap }: RoomViewProps) {
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="bg-charcoal dark:bg-[#0a0a0a] rounded-xl px-4 py-2.5 flex justify-between items-center">
      ...
      {removedSlots.length > 0 && (
        <div className="bg-amber-50 dark:bg-amber-950/40 border-2 border-amber-400 rounded-xl p-3">
          <div className="text-[11px] font-bold text-amber-800 dark:text-amber-400 mb-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {removedSlots.map(({ s, i }) => (
              <div
                key={i}
                className="bg-amber-100 dark:bg-amber-900/50 border border-amber-400 rounded-lg px-2.5 py-1 flex items-center gap-1.5"
              >
                <span className="text-[11px] font-bold text-amber-800 dark:text-amber-400">{slotLabel(i)}</span>
                <span className="text-[11px] text-amber-700 dark:text-amber-300">{s.name}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Fridge grid */}
      <div className="bg-[#9e9e9e] dark:bg-[#1a1a1a] rounded-2xl p-3 shadow-inner">
        {/* Column labels */}
              ...
              <div key={ri} className="grid grid-cols-4 gap-1.5">
                {Array.from({ length: 4 }).map((_, ci) => {
                  ...
                  const headerBg = gone
                    ...
                    : 'bg-[#b0aea8] dark:bg-[#383838]';
                  const headerBorder = gone ? 'border-amber-400' : 'border-[#999] dark:border-[#444]';
                  const bottomBg =
                    gone ? 'bg-amber-800 dark:bg-amber-900' :
                    isRed ? 'bg-red-600' :
                    isOr ? 'bg-orange-600' :
                    ...
                    <button
                      ...
                      className={`rounded-lg border overflow-hidden flex flex-col min-h-[90px] transition-transform active:scale-95 ${
                        gone
                          ...
                          : `bg-[#d0cdc6] dark:bg-[#2a2a2a] border-[#aaa] dark:border-[#3a3a3a] shadow-[inset_0_1px_3px_rgba(255,255,255,.4),0_2px_4px_rgba(0,0,0,.25)] ${isRed ? 'animate-blink-fast' : ''}`
                      }`}
                    >
                      {/* Header */}
                      <div className={`${headerBg} px-1 py-0.5 flex items-center justify-between border-b ${headerBorder}`}>
                        <span className={`text-[7px] font-bold ${gone ? 'text-amber-800 dark:text-amber-400' : isRed ? 'text-red-600' : isOr ? 'text-orange-500' : st === 'ok' ? 'text-green-700 dark:text-green-400' : 'text-gray-500'}`}>
                          {gone ? '取出中' : isRed ? '要交換' : isOr ? '月末' : st === 'ok' ? '販売中' : '未入力'}
                        </span>
                        <div className="flex items-center gap-1">
                          <span className={`text-[8px] font-bold rounded px-0.5 ${gone ? 'bg-amber-200 dark:bg-amber-800 text-amber-800 dark:text-amber-200' : 'bg-black/10 dark:bg-white/10 text-black/40 dark:text-white/40'}`}>
                            {slotLabel(idx)}
                          </span>
                      ...
                      {/* Body */}
                      <div className={`flex-1 flex items-center justify-center p-1 ${gone ? '' : 'bg-[rgba(200,210,220,.4)] dark:bg-[rgba(255,255,255,.04)]'}`}>
                        {gone ? (
                          <div className="flex flex-col items-center gap-0.5">
                            <span className="text-2xl opacity-30">📫</span>
                            <span className="text-[8px] font-bold text-amber-800 dark:text-amber-400 text-center leading-tight">{slot.name}</span>
                          </div>
                        ) : (
                          <div className="bg-white dark:bg-[#2c2c2c] rounded px-1 py-1 w-full min-h-[44px] flex items-center justify-center shadow-sm">
                            <span className={`font-bold text-center leading-tight break-keep ${slot.name ? 'text-[9px] text-[#1a1a1a] dark:text-gray-100' : 'text-[8px] text-gray-300 dark:text-gray-600'}`}>
                              {slot.name || '—'}
                            </span>
                          </div>
                        )}
